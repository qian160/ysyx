...
printf(ANSI_FMT(" ---------------------------------------------------------------------------\n", ANSI_FG_YELLOW));
  ...
  switch(type){  
    ...
    if(D -> decInfo.is_load){
      ...
      IFDEF(CONFIG_MTRACE_ENABLE, update_mringbuf(1, address, loadVal, RD));
    }
    else if(D->decInfo.is_jalr){
      printf(ANSI_FMT("| jalr, set %s = 0x%-lx, new PC at 0x%lx. %s's bits are: \t\t|\n", ANSI_FG_YELLOW), reg_name(RD), src1, src2, reg_name(RD));
      show_bits_fmt(src1);
      /*IFDEF(CONFIG_FTRACE_ENABLE,
        if(RD == 0) update_ftrace(0, src2, "dont know", depth);
      );*/
    }
    else  {
      printf(ANSI_FMT("| set %s = 0x%-60lx  | \n", ANSI_FG_YELLOW), reg_name(RD), R(RD)); 
      show_bits_fmt(R(RD));
    }
    break;
    case(TYPE_B):
    ...
    case(TYPE_S):{
      ...
      IFDEF(CONFIG_MTRACE_ENABLE, update_mringbuf(0, src1, storeVal, RD));
      break;
    }
    default:  break;
  }
...
//question: how to make good use of RD, src1, src2
...
#define INSTPAT_MATCH(D, name, type, ... /* body */ ) { \
  ...
  */
  ...
  INSTPAT("??????? ????? ????? 100 ????? 0010011", xori,     I, R(RD) = SRC1 ^ SRC2);
...
//INSTPAT("0000001 ????? ????? 101 ????? 0110011", divu,     R, R(RD) = (src1 / src2));
  ...
  INSTPAT("??????? ????? ????? 101 ????? 0000011", lhu,      I, R(RD) = Mr(SRC1 + SRC2, 2));
  //store
  ...
  //M extension
  INSTPAT("0000001 ????? ????? 000 ????? 0110011", mul,      R, R(RD) = (sword_t)SRC1 * (sword_t)SRC2);
  INSTPAT("0000001 ????? ????? 001 ????? 0110011", mulh,     R, R(RD) = BITS((__int128_t) SRC1 * (__int128_t) SRC2, 127, 64));
  INSTPAT("0000001 ????? ????? 010 ????? 0110011", mulhsu,   R, R(RD) = BITS((__int128_t) SRC1 * (__uint128_t)SRC2, 127, 64));
  INSTPAT("0000001 ????? ????? 011 ????? 0110011", mulhu,    R, R(RD) = BITS((__uint128_t)SRC1 * (__uint128_t)SRC2, 127, 64));
  ...
  //RV64I
  INSTPAT("0000000 ????? ????? 000 ????? 0111011", addw,     R, R(RD) = SEXT((int) SRC1 + (int) SRC2, 32));
  INSTPAT("0100000 ????? ????? 000 ????? 0111011", subw,     R, R(RD) = SEXT((int) SRC1 - (int) SRC2, 32));
  INSTPAT("0000000 ????? ????? 001 ????? 0111011", sllw,     R, R(RD) = SEXT((int) SRC1 << (int)SRC2, 32));
  INSTPAT("0000000 ????? ????? 101 ????? 0111011", srlw,     R, R(RD) = SEXT((uint32_t)SRC1 >> (uint32_t)SRC2, 32));
  INSTPAT("0100000 ????? ????? 101 ????? 0111011", sraw,     R, R(RD) = SEXT((int)SRC1 >> (int)SRC2, 32));
  INSTPAT("??????? ????? ????? 110 ????? 0000011", lwu,      I, R(RD) = Mr(SRC1 + SRC2, 4));
