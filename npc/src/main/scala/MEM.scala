...
*/
    ...
    val readMask    = MuxLookup(io.memOp_i.length, 0.U, Seq(
    ...
    val loadval = dword & readMask      //before sEXT
    val loadVal = MuxLookup(io.memOp_i.length, 0.U, Seq(
        0.U   ->  Mux(io.memOp_i.sign, SEXT(loadval, 8,  64), loadval),
        1.U   ->  Mux(io.memOp_i.sign, SEXT(loadval, 16, 64), loadval),
        2.U   ->  Mux(io.memOp_i.sign, SEXT(loadval, 32, 64), loadval),
        3.U   ->  loadval,
    ))

    val sdata   =   io.memOp_i.sdata// & mask
    when(isStore){
        /*
