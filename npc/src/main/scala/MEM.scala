...
*/
    ...
    //val load_addr   = io.memOp.addr & ~"b111".U(64.W)       //load 8 aligned bytes from Mem to be used
    ...
    //could be improved..
    //val dword   = Cat(ram(load_addr), ram(load_addr+1.U),ram(load_addr+2.U), ram(load_addr+3.U),ram(load_addr+4.U), ram(load_addr+5.U),ram(load_addr+6.U), ram(load_addr+7.U))
        ...
        */
        switch(io.memOp.length){
            ...
            is(3.U){
                ...
                ram(addr + 1.U) :=  sdata(47, 40)
                ram(addr + 2.U) :=  sdata(55, 48)
                ram(addr + 3.U) :=  sdata(63, 56)
            }
        }
        /*
    ...
    test(0) := ram(CONST.PMEM_START)
    test(1) := ram(CONST.PMEM_START + 1.U)
    test(2) := ram(CONST.PMEM_START + 2.U)
    test(3) := ram(CONST.PMEM_START + 3.U)
    test(4) := ram(CONST.PMEM_START + 4.U)
    test(5) := ram(CONST.PMEM_START + 5.U)
    test(6) := ram(CONST.PMEM_START + 6.U)
    test(7) := ram(CONST.PMEM_START + 7.U)
