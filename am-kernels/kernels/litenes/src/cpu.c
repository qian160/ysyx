...
//byte CPU_RAM[0x8000];   //32KB ?
byte CPU_RAM[0x800];

byte cpu_ram_read(word address) {
  return CPU_RAM[address & 0x7FF];  //2KB ???
}
...
//cpu init will fill these arrays
...
void cpu_run(long cycles) {
  ...
  while (cycles > 0) {
    ...
    /*
        In addressing_mode the PC values will be used to determine op_value and op_address,
      but here we do PC++, so the inst pattern should be [opcode, opval/addr], each 2 Bytes or 3
    */
    if (cpu_op_address_mode[op_code] == NULL) {
    }
    else {
      cpu_op_address_mode[op_code]();   //set op_value and op_address
