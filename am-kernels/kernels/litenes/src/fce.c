...
typedef struct {
  char signature[4];      //0-3: Constant $4E $45 $53 $1A ("NES" followed by MS-DOS end-of-file)
  byte prg_block_count;   //Size of PRG ROM in 16 KB units
  byte chr_block_count;   //Size of CHR ROM in 8 KB units (Value 0 means the board uses CHR RAM)
  word rom_type;
  byte reserved[8];
} ines_header;    //the ines(.nes) file format's header
...
int fce_load_rom(char *rom) {
  buf = (byte*)rom;   //now buf points to the mario ROM
  ...
  int prg_size = fce_rom_header->prg_block_count * 0x4000;  //16KB
